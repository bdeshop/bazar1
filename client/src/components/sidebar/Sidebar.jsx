...
const Sidebar = ({ sidebarOpen, onCategorySelect }) => {
  ...
  return (
    ...
    >
      ...
      <span className={`w-full flex justify-start items-center px-4 pt-4 pb-3 transition-all duration-300 ${sidebarOpen ? 'opacity-100' : 'opacity-0 h-0 p-0 mb-0'}`}>
        {sidebarOpen ? (
      ...
      {
        !sidebarOpen ?           <span className="bg-theme_gray p-2 rounded-[3px] text-center flex justify-center items-center gap-3">
            <MdSupportAgent className="text-white text-[20px]" />
          </span>:""
      }
     
      <div className={sidebarOpen ? "p-[10px]" : "hidden"}>
        <img className="w-full" src="https://img.b112j.com/upload/h5Announcement/image_182702.jpg" alt="" />
      </div>
        ...
        {categories.map((category, index) => (
          <div key={category._id}>
            ...
            >
              ...
              />
              <div className={`flex items-center overflow-hidden transition-all duration-300 ${sidebarOpen ? 'ml-3 w-full' : 'w-0'}`}>
            ...
            <div 
              className={`overflow-hidden transition-all duration-300 ease-in-out ${
                sidebarOpen && activeMenu === category.name && category.providers && category.providers.length > 0 ? 
                'max-h-96' : 'max-h-0'
              }`}
            >
      ...
      >
        <div className="flex justify-between items-center p-2">
          ...
          <span className="text-xs text-theme_color2 underline cursor-pointer">View all</span>
        </div>
      </div>
      ...
      <div className="space-y-1 ">
        {secondaryMenuItems.map((item, index) => (
          <div key={index}>
            <div 
              className={`flex items-center p-3 rounded text-gray-500 cursor-pointer hover:text-gray-600 transition-colors duration-200 ${activeMenu === item.title ? 'bg-[#333]' : ''}`}
            ...
            <div 
              className={`overflow-hidden transition-all duration-300 ease-in-out ${
                sidebarOpen && activeMenu === item.title && item.subItems.length > 0 ? 'max-h-96' : 'max-h-0'
              }`}
            >
              {sidebarOpen && (
                <div className="ml-8 mt-1 mb-2 space-y-1">
