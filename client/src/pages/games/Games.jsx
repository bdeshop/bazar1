...
const Games = () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (categoryParam && categoryParam !== 'all') {
          ...
          filtered = filtered.filter(game =>
            game.category.toLowerCase() === 'casino'
          );
        }
  ...
  const handleCategoryChange = (categoryValue) => {
    ...
    if (categoryValue !== 'all') {
      ...
      setProviders([{ name: "All Providers", value: "all", icon: "fas fa-grid" }]);
    }
        ...
        <div className={`flex-1 overflow-auto transition-all duration-300 ${isLoading ? 'opacity-50' : ''}`}>
          <div className='mx-auto w-full max-w-screen-xl py-4 px-4 sm:px-6 md:px-8 lg:px-12'>
            <div className="flex items-center justify-between mb-6">
              <h1 className="text-base sm:text-lg md:text-xl font-semibold text-white">Casino Games</h1>
      ...
      {showFilterSidebar && (
        <div ref={filterSidebarRef} className={`fixed pt-6 top-0 right-0 h-full ${isMobile ? 'left-0 w-full' : 'w-80'} bg-[#0f0f0f] z-50 shadow-lg overflow-y-auto flex flex-col`}>
          ...
          <div className="flex-1 p-4">
            <div className="mb-4">
              <label className="block text-sm font-medium mb-3 text-gray-300">Providers</label>
