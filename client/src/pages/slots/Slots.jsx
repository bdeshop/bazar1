...
const SlotsContent = () => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const slotsCategory = categoriesData.find(
          (cat) => cat.value === "slots"
        );
        setSelectedCategory(
          slotsCategory ? "slots" : categoriesData[0]?.value || "all"
        );
      }
    } catch (error) {
  ...
  const handleOpenGame = async (game) => {
    ...
    try {
      setGameLoading(true);
      const response = await axios.post(`${base_url}/api/user/play-game`, {
        gameID: game.gameID,
        slug: "/api/route",
                  ...
                  <div className="flex items-center">
                    {categories.find((c) => c.value === selectedCategory)
                      ...
                      <img
                        ...
                        alt=""
                        className="mr-2 w-4 h-4 md:w-7 md:h-7"
                      />
                    )}
                    <i
                      className={`${
