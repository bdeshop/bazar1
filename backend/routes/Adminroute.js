...
Adminrouter.get('/dashboard', async (req, res) => {
  try {
    ...
    const activeCategories = await GameCategory.countDocuments({ status: true });
    const inactiveCategories = await GameCategory.countDocuments({ status: false });
    ...
    const inactiveProviders = await GameProvider.countDocuments({ status: false });

    // 8. PROMOTIONAL CONTENT STATISTICS
    const totalPromotionals = await Promotional.countDocuments();
    const activePromotionals = await Promotional.countDocuments({ status: true });
    const inactivePromotionals = await Promotional.countDocuments({ status: false });
...
Adminrouter.get('/users', async (req, res) => {
  try {
    ...
    const users = await User.find(filter)
      ...
      .select('-password -transactionPassword -moneyTransferPassword -twoFactorSecret');
...
Adminrouter.put('/users/:id/status', async (req, res) => {
  try {
    ...
    if (!status || !['active', 'banned', 'deactivated', 'pending'].includes(status)) {
...
Adminrouter.get('/users/:id/financial-stats', async (req, res) => {
  try {
    const user = await User.findById(req.params.id)
      .select('total_deposit total_withdraw total_bet total_wins total_loss net_profit lifetime_deposit lifetime_withdraw lifetime_bet');
...
Adminrouter.get('/users/:id/bonus-info', async (req, res) => {
  try {
    const user = await User.findById(req.params.id)
      .select('bonusInfo bonusBalance bonusActivityLogs');
...
Adminrouter.post('/users/:id/add-bonus', async (req, res) => {
  try {
    ...
    if (!bonusType || !amount || amount <= 0) {
      return res.status(400).json({ error: 'Valid bonus type and amount are required' });
    }
...
Adminrouter.post('/users/:id/adjust-balance', async (req, res) => {
  try {
    ...
    if (!amount || amount <= 0 || !type || !['add', 'subtract'].includes(type)) {
...
Adminrouter.get('/users/:id/referral-info', async (req, res) => {
  try {
    const user = await User.findById(req.params.id)
      .select('referralCode referralEarnings referralCount referralUsers referralTracking')
...
Adminrouter.get('/users/:id/login-history', async (req, res) => {
  try {
    ...
    const user = await User.findById(req.params.id)
      .select('loginHistory login_count last_login');
...
Adminrouter.get('/users/:id/kyc-info', async (req, res) => {
  try {
    const user = await User.findById(req.params.id)
      .select('kycStatus kycDocuments isEmailVerified isPhoneVerified');
...
Adminrouter.put('/users/:id/kyc-status', async (req, res) => {
  try {
    ...
    if (!kycStatus || !['unverified', 'pending', 'verified', 'rejected'].includes(kycStatus)) {
...
Adminrouter.get('/users/:id/wagering-status', async (req, res) => {
  try {
    const user = await User.findById(req.params.id)
      .select('total_deposit totalWagered bonusInfo');
...
Adminrouter.post('/banners', uploadBanners.array('images', 5), async (req, res) => {
  try {
...
// PUT update banner
Adminrouter.put('/banners/:id', uploadBanners.single('image'), async (req, res) => {
  try {
    const banner = await Banner.findById(req.params.id);
    if (!banner) {
      return res.status(404).json({ error: 'Banner not found' });
...
Adminrouter.post('/promotionals', uploadPromotional.single('image'), async (req, res) => {
  try {
    if (!req.file) {
...
Adminrouter.put('/promotionals/:id/status', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to update promotional content status' });
  }
});

// PUT update promotional content
Adminrouter.put('/promotionals/:id', uploadPromotional.single('image'), async (req, res) => {
  try {
    const promotional = await Promotional.findById(req.params.id);
    if (!promotional) {
      return res.status(404).json({ error: 'Promotional content not found' });
    ...
    if (req.body.targetUrl !== undefined) promotional.targetUrl = req.body.targetUrl;
  ...
  try {
    ...
    if (!terms) {
      terms = new Terms({
        ...
        content: 'Please replace this with your actual terms and conditions content.',
...
Adminrouter.get('/terms/history', async (req, res) => {
  try {
    const terms = await Terms.findOne().populate('history.updatedBy', 'name email');
...
Adminrouter.put('/terms', async (req, res) => {
  try {
    ...
    if (!title || !content || !lastUpdated) {
      return res.status(400).json({ error: 'Title, content, and lastUpdated are required' });
    }
...
Adminrouter.get('/game-categories', async (req, res) => {
  try {
    ...
    const categories = await GameCategory.find(filter).sort({ order: 1, createdAt: -1 });
    res.json(categories);
  } catch (error) {
...
Adminrouter.post('/game-categories', uploadGameCategory.single('image'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'Please upload a category image' });
    }
...
Adminrouter.put('/game-categories/:id', uploadGameCategory.single('image'), async (req, res) => {
  try {
    const category = await GameCategory.findById(req.params.id);
    if (!category) {
      return res.status(404).json({ error: 'Game category not found' });
...
Adminrouter.get('/game-providers', async (req, res) => {
  try {
    ...
    const providers = await GameProvider.find(filter).sort({ order: 1, createdAt: -1 });
    res.json(providers);
  } catch (error) {
...
Adminrouter.post('/game-providers', uploadGameProvider.single('image'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ error: 'Please upload a provider image' });
    }

    if (!req.body.name || !req.body.website) {
      return res.status(400).json({ error: 'Provider name and website are required' });
    }
...
Adminrouter.put('/game-providers/:id', uploadGameProvider.single('image'), async (req, res) => {
  try {
    const provider = await GameProvider.findById(req.params.id);
    if (!provider) {
      return res.status(404).json({ error: 'Game provider not found' });
...
Adminrouter.post('/games', uploadGameImages.fields([
  { name: 'portraitImage', maxCount: 1 },
  { name: 'landscapeImage', maxCount: 1 }
]), async (req, res) => {
  try {
    const { name, gameId, provider, category, featured, status } = req.body;
    ...
    if (!req.files || !req.files.portraitImage || !req.files.landscapeImage) {
      return res.status(400).json({ error: 'Both portrait and landscape images are required' });
    }
...
// PUT update game
Adminrouter.put('/games/:id', uploadGameImages.fields([
  { name: 'portraitImage', maxCount: 1 },
  { name: 'landscapeImage', maxCount: 1 }
]), async (req, res) => {
  try {
    ...
    if (req.body.gameId) {
      // Check if new gameId already exists (excluding current game)
    ...
    if (req.files && req.files.portraitImage) {
      ...
      if (game.portraitImage) {
        const oldImagePath = path.join(__dirname, '..', game.portraitImage);
        if (fs.existsSync(oldImagePath)) {
          fs.unlinkSync(oldImagePath);
        }
      }
    ...
    if (req.files && req.files.landscapeImage) {
      ...
      if (game.landscapeImage) {
        const oldImagePath = path.join(__dirname, '..', game.landscapeImage);
        if (fs.existsSync(oldImagePath)) {
          fs.unlinkSync(oldImagePath);
        }
      }
...
Adminrouter.put('/deposits/:id/status', async (req, res) => {
  try {
    ...
    const deposit = await Deposit.findById(req.params.id)
      .populate('userId', 'username player_id balance');
    ...
    if (oldStatus === 'completed' && status !== 'completed') {
      ...
      if (user.balance < deposit.amount) {
        return res.status(400).json({ error: 'User has insufficient balance to reverse this deposit' });
      }
...
Adminrouter.put('/deposits/:id', async (req, res) => {
  try {
    ...
    if (oldStatus === 'approved' && amount !== undefined && amount !== oldAmount) {
      ...
      if (amountDifference < 0 && user.balance < Math.abs(amountDifference)) {
        return res.status(400).json({ error: 'User has insufficient balance for this adjustment' });
      }
...
Adminrouter.delete('/deposits/:id', async (req, res) => {
  try {
    ...
    if (deposit.status === 'approved') {
      ...
      // Check if user has sufficient balance
      if (user.balance < deposit.amount) {
        return res.status(400).json({ error: 'User has insufficient balance to delete this deposit' });
      }
...
Adminrouter.get('/deposits-stats', async (req, res) => {
  try {
    ...
    res.json({
      total: totalStats[0] || { totalCount: 0, totalAmount: 0, averageAmount: 0 },
...
Adminrouter.post('/deposits/manual', async (req, res) => {
  try {
    const { userId, amount, method, phoneNumber, transactionId, notes } = req.body;
...
Adminrouter.get('/deposits/export', async (req, res) => {
  try {
    ...
    let csv = 'Date,Username,Player ID,Method,Amount,Status,Transaction ID,Phone Number\n';
    
    deposits.forEach(deposit => {
      csv += `"${new Date(deposit.createdAt).toISOString()}","${deposit.userId.username}","${deposit.userId.player_id}","${deposit.method}","${deposit.amount}","${deposit.status}","${deposit.transactionId || 'N/A'}","${deposit.phoneNumber || 'N/A'}"\n`;
    });
...
Adminrouter.put('/withdrawals/:id', async (req, res) => {
  try {
    ...
    if (oldStatus === 'completed' && amount !== undefined && amount !== oldAmount) {
...
Adminrouter.get('/users/:userId/withdrawals', async (req, res) => {
  try {
    ...
    // Get user information
    const user = await User.findById(userId).select('username player_id balance');
...
Adminrouter.post('/withdrawals/manual', async (req, res) => {
  try {
    ...
    if (!userId || !amount || !method || !phoneNumber) {
      return res.status(400).json({ error: 'User ID, amount, method, and phone number are required' });
    }
...
Adminrouter.get('/withdrawals/export', async (req, res) => {
  try {
    ...
    // Convert to CSV format
    let csv = 'Date,Username,Player ID,Method,Amount,Status,Transaction ID,Phone Number,Processed At\n';
    
    withdrawals.forEach(withdrawal => {
      csv += `"${new Date(withdrawal.createdAt).toISOString()}","${withdrawal.userId.username}","${withdrawal.userId.player_id}","${withdrawal.method}","${withdrawal.amount}","${withdrawal.status}","${withdrawal.transactionId || 'N/A'}","${withdrawal.phoneNumber || 'N/A'}","${withdrawal.processedAt ? new Date(withdrawal.processedAt).toISOString() : 'N/A'}"\n`;
    });
...
Adminrouter.put('/withdrawals/bulk/status', async (req, res) => {
  try {
    ...
    if (!withdrawalIds || !Array.isArray(withdrawalIds) || withdrawalIds.length === 0) {
      return res.status(400).json({ error: 'Withdrawal IDs array is required' });
    }
    ...
    const result = await Withdrawal.updateMany(
      ...
      { 
        status,
        ...(status === 'processing' || status === 'completed' ? { processedAt: new Date() } : {}),
...
const brandingStorage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    ...
    const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
    const type = file.fieldname === 'logo' ? 'logo' : 'favicon';
    cb(null, `branding-${type}-${uniqueSuffix}${path.extname(file.originalname)}`);
  }
});
...
Adminrouter.post('/upload-branding', uploadBranding.fields([
...
Adminrouter.post('/notifications', async (req, res) => {
  try {
    ...
    if (targetType === 'specific' && (!targetUsers || targetUsers.length === 0)) {
      return res.status(400).json({ error: 'Target users are required for specific notifications' });
    }
    
    if (targetType === 'role_based' && (!userRoles || userRoles.length === 0)) {
      return res.status(400).json({ error: 'User roles are required for role-based notifications' });
    }
    
    // Validate target users exist if provided
    if (targetType === 'specific' && targetUsers.length > 0) {
      const usersExist = await User.countDocuments({ _id: { $in: targetUsers } });
      if (usersExist !== targetUsers.length) {
        return res.status(400).json({ error: 'One or more target users do not exist' });
      }
    }
...
Adminrouter.get('/notifications-stats', async (req, res) => {
  try {
    ...
    res.json({
      total: totalStats[0] || { totalCount: 0, scheduledCount: 0, sentCount: 0 },
      byType: typeStats,
...
Adminrouter.get('/login-logs/stats', async (req, res) => {
  try {
    ...
    res.json({
      total: totalStats[0] || { totalAttempts: 0, successfulAttempts: 0, failedAttempts: 0 },
...
Adminrouter.post('/ip-whitelist', async (req, res) => {
  try {
    ...
    if (!ipAddress || !description) {
      return res.status(400).json({ error: 'IP address and description are required' });
    }
...
Adminrouter.put('/security-settings/:userId', async (req, res) => {
  try {
    ...
    if (twoFactorMethod !== undefined) securitySettings.twoFactorMethod = twoFactorMethod;
    if (loginAlerts !== undefined) securitySettings.loginAlerts = loginAlerts;
    if (suspiciousActivityAlerts !== undefined) securitySettings.suspiciousActivityAlerts = suspiciousActivityAlerts;
    if (sessionTimeout !== undefined) securitySettings.sessionTimeout = sessionTimeout;
    if (maxFailedAttempts !== undefined) securitySettings.maxFailedAttempts = maxFailedAttempts;
    if (accountLockoutTime !== undefined) securitySettings.accountLockoutTime = accountLockoutTime;
    if (ipWhitelisting !== undefined) securitySettings.ipWhitelisting = ipWhitelisting;
    if (deviceWhitelisting !== undefined) securitySettings.deviceWhitelisting = deviceWhitelisting;
    if (passwordChangeReminder !== undefined) securitySettings.passwordChangeReminder = passwordChangeReminder;
    if (passwordChangeFrequency !== undefined) securitySettings.passwordChangeFrequency = passwordChangeFrequency;
...
Adminrouter.put('/security-settings/:userId/password-change', async (req, res) => {
  try {
    const { userId } = req.params;
...
Adminrouter.get('/security-settings-stats', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch security settings statistics' });
  }
});
...
Adminrouter.get('/events', async (req, res) => {
  try {
    const events = await Event.find().populate('createdBy', 'username fullName').sort({ createdAt: -1 });
    res.json({
      success: true,
      data: events
    });
...
// PUT update event
Adminrouter.put('/events/:id', uploadEvent.single('image'), async (req, res) => {
  try {
