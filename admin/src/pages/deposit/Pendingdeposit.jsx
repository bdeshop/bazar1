...
import { FaSearch, FaFilter, FaEye, FaSort, FaSortUp, FaSortDown, FaMoneyBill, FaClock, FaCheckCircle, FaTimesCircle, FaExclamationTriangle, FaDownload, FaEdit, FaTrash } from 'react-icons/fa';
...
const Pendingdeposit = () => {
  ...
  const methods = ['all', 'bkash', 'nagad', 'rocket', 'bank_transfer', 'upay', 'card'];
  ...
  const fetchPendingDeposits = async () => {
    try {
      ...
      setStats({
        ...
        pendingAmount: response.data.statusCounts.find(s => s._id === 'pending')?.amount || 0
      });
    } catch (err) {
      ...
      setDeposits([
        {
          _id: { $oid: "68ae24b8c2b1c27dfe6572c2" },
          ...
          processed_by: null,
          processed_at: null,
          created_at: { $date: "2025-08-27T10:15:30.904Z" },
          notes: "Waiting for confirmation",
          fee: 25,
          net_amount: 9975
        },
        {
          _id: { $oid: "68ae24b8c2b1c27dfe6572c7" },
          userId: { username: "sarahsmith", player_id: "PID507956" },
          amount: 45000,
          currency: "BDT",
          method: "nagad",
          phoneNumber: "01798765432",
          transaction_id: "TX258456963",
          status: "pending",
          processed_by: null,
          processed_at: null,
          created_at: { $date: "2025-08-29T12:10:27.904Z" },
          notes: "Large amount pending verification",
          fee: 45,
          net_amount: 44955
        }
      ]);
  ...
  const fetchStats = async () => {
    try {
      const token = localStorage.getItem('uber');
      const response = await axios.get(`${API_BASE_URL}/api/admin/deposits-stats?status=pending`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
      ...
      setStats({
        ...
        pendingAmount: response.data.byStatus.find(s => s._id === 'pending')?.amount || 0
      });
    } catch (err) {
  ...
  const sortedDeposits = React.useMemo(() => {
    ...
    if (sortConfig.key !== null) {
      sortableItems.sort((a, b) => {
        let aValue = sortConfig.key === 'username' ? a.userId[sortConfig.key] : a[sortConfig.key];
        let bValue = sortConfig.key === 'username' ? b.userId[sortConfig.key] : b[sortConfig.key];

        if (sortConfig.key === 'created_at' || sortConfig.key === 'processed_at') {
          aValue = aValue ? new Date(aValue.$date) : new Date(0);
          bValue = bValue ? new Date(bValue.$date) : new Date(0);
        }
  ...
  // Filter deposits based on search and method
  const filteredDeposits = sortedDeposits.filter(deposit => {
    const matchesSearch = deposit.userId.username.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    if (dateRange.start && dateRange.end) {
      const depositDate = new Date(deposit.created_at.$date);
  ...
  const formatDate = (dateObj) => {
    ...
    return new Date(dateObj.$date).toLocaleDateString('en-BD', {
      year: 'numeric',
      month: 'short',
      day: 'numeric',
      hour: '2-digit',
  ...
  const exportToCSV = async () => {
    try {
      const token = localStorage.getItem('token');
      let url = `${API_BASE_URL}/api/admin/deposits/export?status=pending`;
  ...
  return (
    <section className="font-nunito h-screen">
      ...
      <div className="flex pt-[10vh]">
        <Sidebar isOpen={isSidebarOpen} />
        <main
          className={`transition-all duration-300 flex-1 p-6 overflow-y-auto h-[90vh] ${isSidebarOpen ? 'md:ml-[40%] lg:ml-[28%] xl:ml-[17%] ' : 'ml-0'}`}
        >
          <div className="w-full mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">Manage pending deposit transactions</p>
              </div>
              <button
                onClick={exportToCSV}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
              <div className="bg-white p-4 rounded-[5px] shadow around-sm border-[1px] border-gray-200">
                <h3 className="text-sm font-medium text-gray-600">Total Pending Deposits</h3>
            ...
            <div className="mb-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-2">
              <p className="text-gray-600">
                Showing {filteredDeposits.length} of {stats.total} pending deposits
              </p>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {currentItems.length > 0 ? (
                      currentItems.map((deposit) => {
                        ...
                        return (
                          <tr key={deposit._id.$oid} className="hover:bg-gray-50 transition-colors duration-150">
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-700">{formatDate(deposit.created_at)}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm font-semibold text-gray-700 fontύπ4 font-mono">{deposit.userId.player_id}</div>
                              <div className="text-xs text-gray-500">{deposit.userId.username}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-700">{formatCurrency(deposit.fee)}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm font-medium text-green-600">{formatCurrency(deposit.net_amount)}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-700">{deposit.phoneNumber || 'N/A'}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-700 font-mono bg-gray-50 px-2 py-1 rounded border border-gray-200">{deposit.transaction_id}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                              ...
                              <button
                                ...
                                onClick={() => deleteDeposit(deposit._id.$oid)}
                              >
                                <FaTrash />
                              </button>
                            </td>
                      ...
                      <tr>
                        <td colSpan="10" className="px-6 py-12 text-center">
                          <div className="flex flex-col items-center justify-center text-gray-400">
                            ...
                            <p className="text-lg font-medium text-gray-500">No pending deposits found</p>
            ...
            {filteredDeposits.length > itemsPerPage && (
              <div className="flex items-center justify-between mt-4 px-4 py-3">
                <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                  ...
                  <div>
                    <nav className="relative z-0 inline-flex rounded}u rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                      <button
        ...
        <div className="fixed inset-0 bg-[rgba(0,0,0,0.4)] bg-opacity-50 flex items-center justify-center z-[1000] backdrop-blur-md p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
              <h3 className="text-lg font-medium text-gray-900">Pending Deposit Details</h3>
            ...
            <div className="px-6 py-4">
                ...
                <div>
                  ...
                  <dl className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <dd className="text-sm font-medium text-gray-900">{selectedDeposit.transaction_id || 'N/A'}</dd>
                    </div>
                    <div className="flex justify-between">
                      <dt className="text-sm text-gray-600">Date & Time:</dt>
                      <dd className="text-sm text-gray-900">{formatDate(selectedDeposit.created_at)}</dd>
                    </div>
                    <div className="flex justify-between">
                    ...
                    {selectedDeposit.processed_at && (
                      <div className="flex justify-between">
                        ...
                        <dd className="text-sm text-gray-900">{formatDate(selectedDeposit.processed_at)}</dd>
                      </div>
                    )}
                  </dl>
                </div>
              ...
              <div className="bg-gray-50 p-4 rounded-md mb-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm text-gray-900">{formatCurrency(selectedDeposit.fee)}</span>
                  </div>
                  <div className="flex justify-between border-t border-gray-200 pt-2">
                    <span className="text-sm font-medium text-gray-700">Net Amount:</span>
                    <span className="text-lg font-bold text-green-600">{formatCurrency(selectedDeposit.net_amount)}</span>
                  </div>
                </div>
              </div>
              <div className="mb-6">
                <h4 className="text-sm font-medium text-gray-700 mb-2">Status</h4>
                <div className={`px-4 py-2 inline-flex items-center rounded-md border ${getStatusInfo(deposit.status).color}`}>
              ...
              {selectedDeposit.notes && (
                <div>
                  ...
                  <p className="text-sm text-gray-600 bg-gray-50 p-3 rounded-md">{selectedDeposit.notes}</p>
                </div>
              )}
              {/* Status Actions */}
              <div className="mt-6 space-x-3">
                <button
                  onClick={() => updateDepositStatus(selectedDeposit._id.$oid, 'approved', 'Deposit approved by admin')}
                  className="px-4 py-2 bg-green-600 text-white rounded-md text-sm font-medium hover:bg-green-700"
                >
                  Approve
                </button>
                <button
                  onClick={() => updateDepositStatus(selectedDeposit._id.$oid, 'rejected', 'Deposit rejected by admin')}
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            ...
            <div className="px-6 py-4">
              <form onSubmit={(e) => {
                ...
                editDeposit(selectedDeposit._id.$oid, updates);
              }}>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      name="transactionId"
                      defaultValue={selectedDeposit.transaction_id}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500"
                    />
                  </div>
                  <div>
